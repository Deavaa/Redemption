...
<div class="modern-page">
    ...
    <div class="modern-page-header">
        <div class="modern-page-header-left">
            ...
            <h1 class="modern-page-title">Performance Reports</h1>
            <p class="modern-page-subtitle">Track student behavioral, athletic and extracurricular performance</p>
        </div>
        <div class="modern-page-header-right">
            <a href="{{ route('admin.performance-reports.create') }}" class="btn-modern btn-modern-primary">
    ...
    <div class="modern-stats-row">
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                ...
                <span class="modern-stat-label">Total Reports</span>
            </div>
        </div>
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <span class="modern-stat-value">{{ $data->where('overall_rating', 'excellent')->count() }}</span>
                <span class="modern-stat-label">Excellent</span>
            </div>
        </div>
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <span class="modern-stat-value">{{ $data->avg('attendance_percentage') ? number_format($data->avg('attendance_percentage'), 1) : '0' }}%</span>
                <span class="modern-stat-label">Avg Attendance</span>
            </div>
        </div>
    </div>
    ...
    <div class="modern-card">
        <div class="modern-card-header">
            <div class="modern-card-header-left">
                <h2 class="modern-card-title">All Performance Reports</h2>
                <span class="modern-badge modern-badge-light">{{ $data->total() }} records</span>
            </div>
            <div class="modern-card-header-right">
                <div class="modern-search-box">
                    <i class="fas fa-search"></i>
                    <input type="text" id="reportSearch" placeholder="Search reports..." onkeyup="filterTable()">
                </div>
            </div>
        </div>
        <div class="modern-card-body">
                ...
                <table class="modern-table" id="reportTable">
                    <thead>
                        <tr>
                            ...
                            <th class="th-center">Behavior</th>
                            <th class="th-center">Sports</th>
                            <th class="th-center">Extracurricular</th>
                            <th class="th-center">Overall</th>
                    ...
                    <tbody>
                        ...
                        <tr class="modern-table-row">
                            ...
                            <td class="td-center">
                                @if($item->attendance_percentage !== null)
                                    <span class="modern-cell-bold">{{ number_format($item->attendance_percentage, 1) }}%</span>
                                @else
                                    <span class="modern-cell-muted">-</span>
                                @endif
                            </td>
                            <td class="td-center">
                                <span class="modern-badge {{ $item->behavior_rating === 'excellent' ? 'modern-badge-success' : ($item->behavior_rating === 'good' ? 'modern-badge-info' : ($item->behavior_rating === 'average' ? 'modern-badge-gold' : 'modern-badge-danger')) }}">
                                    {{ ucfirst($item->behavior_rating ?? '-') }}
                                </span>
                            </td>
                            <td class="td-center">
                                <span class="modern-badge {{ $item->sports_rating === 'excellent' ? 'modern-badge-success' : ($item->sports_rating === 'good' ? 'modern-badge-info' : ($item->sports_rating === 'average' ? 'modern-badge-gold' : 'modern-badge-danger')) }}">
                                    {{ ucfirst($item->sports_rating ?? '-') }}
                                </span>
                            </td>
                            <td class="td-center">
                                <span class="modern-badge {{ $item->extracurricular_rating === 'excellent' ? 'modern-badge-success' : ($item->extracurricular_rating === 'good' ? 'modern-badge-info' : ($item->extracurricular_rating === 'average' ? 'modern-badge-gold' : 'modern-badge-danger')) }}">
                                    {{ ucfirst($item->extracurricular_rating ?? '-') }}
                                </span>
                            </td>
                            <td class="td-center">
                                <span class="modern-badge {{ $item->overall_rating === 'excellent' ? 'modern-badge-success' : ($item->overall_rating === 'good' ? 'modern-badge-info' : ($item->overall_rating === 'average' ? 'modern-badge-gold' : 'modern-badge-danger')) }}">
                                    {{ ucfirst($item->overall_rating ?? '-') }}
                                </span>
                            </td>
                            <td class="td-actions">
                                <div class="modern-action-group">
                                    <a href="{{ route('admin.performance-reports.edit', $item->id) }}" class="modern-btn-icon modern-btn-edit" title="Edit">
                                        <i class="fas fa-pen"></i>
                                    </a>
                                    <form method="POST" action="{{ route('admin.performance-reports.destroy', $item->id) }}" style="display:inline" onsubmit="return confirm('Are you sure you want to delete this report?')">
                                        @csrf @method('DELETE')
            ...
            <div class="modern-empty-state">
                ...
                <h3>No Performance Reports Yet</h3>
                <p>Get started by adding the first student performance report.</p>
                <a href="{{ route('admin.performance-reports.create') }}" class="btn-modern btn-modern-primary">
