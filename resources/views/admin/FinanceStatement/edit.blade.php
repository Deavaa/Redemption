...
@section('title', 'Edit Finance Statement')
...
<div class="modern-page">
        ...
        <div class="modern-page-header-left">
            <nav aria-label="breadcrumb" class="modern-breadcrumb">
                <ol>
                    ...
                    <li><a href="{{ route('admin.finance-statements.index') }}">Statements</a></li>
                    <li class="active">Edit</li>
                </ol>
            </nav>
            <h1 class="modern-page-title">Edit Finance Statement</h1>
            <p class="modern-page-subtitle">Update financial statement for <strong>{{ ucfirst(str_replace('_', ' ', $item->statement_type ?? '')) }}</strong></p>
        </div>
        <div class="modern-page-header-right">
            <a href="{{ route('admin.finance-statements.index') }}" class="btn-modern btn-modern-outline">
    ...
    <div class="modern-card">
        <form method="POST" action="{{ route('admin.finance-statements.update', $item->id) }}">
            @csrf @method('PUT')
            ...
            <div class="modern-form-section">
                <div class="modern-form-section-header">
                    ...
                    <div>
                        <h3 class="modern-form-section-title">Statement Details</h3>
                        <p class="modern-form-section-desc">Select the statement type and academic period</p>
                    </div>
                </div>
                <div class="modern-form-section-body">
                    <div class="modern-form-grid">
                        <div class="modern-form-group">
                            <label class="modern-form-label" for="statement_type">
                                Statement Type <span class="modern-required">*</span>
                            </label>
                            <div class="modern-input-wrapper">
                                <i class="fas fa-list-alt modern-input-icon"></i>
                                <select name="statement_type" id="statement_type" class="modern-input modern-select {{ $errors->has('statement_type') ? 'is-invalid' : '' }}" required>
                                    <option value="">-- Select Type --</option>
                                    <option value="income_statement" {{ old('statement_type', $item->statement_type) === 'income_statement' ? 'selected' : '' }}>Income Statement</option>
                                    <option value="balance_sheet" {{ old('statement_type', $item->statement_type) === 'balance_sheet' ? 'selected' : '' }}>Balance Sheet</option>
                                    <option value="cash_flow" {{ old('statement_type', $item->statement_type) === 'cash_flow' ? 'selected' : '' }}>Cash Flow</option>
                                    <option value="trial_balance" {{ old('statement_type', $item->statement_type) === 'trial_balance' ? 'selected' : '' }}>Trial Balance</option>
                                </select>
                            </div>
                            ...
                            <div class="modern-input-wrapper">
                                ...
                                <select name="academic_year_id" id="academic_year_id" class="modern-input modern-select {{ $errors->has('academic_year_id') ? 'is-invalid' : '' }}" required>
                                    ...
                                    @foreach(\App\Models\AcademicYear::orderBy('name', 'desc')->get() as $year)
                                        <option value="{{ $year->id }}" {{ old('academic_year_id', $item->academic_year_id) == $year->id ? 'selected' : '' }}>
                                            {{ $year->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="modern-input-wrapper">
                                ...
                                <select name="branch_id" id="branch_id" class="modern-input modern-select">
                                    ...
                                    @foreach(\App\Models\Branch::orderBy('name')->get() as $branch)
                                        <option value="{{ $branch->id }}" {{ old('branch_id', $item->branch_id) == $branch->id ? 'selected' : '' }}>
                                            {{ $branch->name }}
                                        </option>
                                    @endforeach
                                </select>
            ...
            <div class="modern-form-section">
                <div class="modern-form-section-header">
                    ...
                    <div>
                        <h3 class="modern-form-section-title">Reporting Period</h3>
                        <p class="modern-form-section-desc">Define the date range for this statement</p>
                    </div>
                </div>
                <div class="modern-form-section-body">
                    <div class="modern-form-grid">
                        <div class="modern-form-group">
                            <label class="modern-form-label" for="period_from">
                                Period From <span class="modern-required">*</span>
                            </label>
                            <div class="modern-input-wrapper">
                                <i class="fas fa-calendar-plus modern-input-icon"></i>
                                <input type="date" name="period_from" id="period_from"
                                    class="modern-input {{ $errors->has('period_from') ? 'is-invalid' : '' }}"
                                    value="{{ old('period_from', $item->period_from ? $item->period_from->format('Y-m-d') : '') }}"
                                    required>
                            </div>
                            @error('period_from')
                                <span class="modern-form-error">{{ $message }}</span>
                        ...
                        <div class="modern-form-group">
                            <label class="modern-form-label" for="period_to">
                                Period To <span class="modern-required">*</span>
                            </label>
                            <div class="modern-input-wrapper">
                                <i class="fas fa-calendar-minus modern-input-icon"></i>
                                <input type="date" name="period_to" id="period_to"
                                    class="modern-input {{ $errors->has('period_to') ? 'is-invalid' : '' }}"
                                    value="{{ old('period_to', $item->period_to ? $item->period_to->format('Y-m-d') : '') }}"
                                    required>
                            </div>
                            @error('period_to')
                                <span class="modern-form-error">{{ $message }}</span>
            ...
            <div class="modern-form-section">
                <div class="modern-form-section-header">
                    ...
                    <div>
                        <h3 class="modern-form-section-title">Financial Figures</h3>
                        <p class="modern-form-section-desc">Enter the income, expense and net balance amounts</p>
                    </div>
                </div>
                <div class="modern-form-section-body">
                    <div class="modern-form-grid">
                        <div class="modern-form-group">
                            <label class="modern-form-label" for="total_income">
                                Total Income <span class="modern-required">*</span>
                            </label>
                            <div class="modern-input-wrapper">
                                <i class="fas fa-arrow-up modern-input-icon"></i>
                                <input type="number" step="0.01" min="0" name="total_income" id="total_income"
                                    class="modern-input {{ $errors->has('total_income') ? 'is-invalid' : '' }}"
                                    value="{{ old('total_income', $item->total_income) }}"
                        ...
                        <div class="modern-form-group">
                            <label class="modern-form-label" for="total_expense">
                                Total Expense <span class="modern-required">*</span>
                            </label>
                            <div class="modern-input-wrapper">
                                <i class="fas fa-arrow-down modern-input-icon"></i>
                                <input type="number" step="0.01" min="0" name="total_expense" id="total_expense"
                                    class="modern-input {{ $errors->has('total_expense') ? 'is-invalid' : '' }}"
                                    value="{{ old('total_expense', $item->total_expense) }}"
                        ...
                        <div class="modern-form-group">
                            <label class="modern-form-label" for="net_balance">
                                Net Balance <span class="modern-required">*</span>
                            </label>
                            <div class="modern-input-wrapper">
                                <i class="fas fa-balance-scale modern-input-icon"></i>
                                <input type="number" step="0.01" name="net_balance" id="net_balance"
                                    class="modern-input {{ $errors->has('net_balance') ? 'is-invalid' : '' }}"
                                    value="{{ old('net_balance', $item->net_balance) }}"
            ...
            <div class="modern-form-section">
                <div class="modern-form-section-header">
                    ...
                    <div>
                        ...
                        <p class="modern-form-section-desc">Additional notes about this financial statement</p>
                    </div>
                </div>
                <div class="modern-form-section-body">
                    <div class="modern-form-grid">
                        <div class="modern-form-group modern-form-span-2">
                            ...
                            <div class="modern-input-wrapper">
                                ...
                                <textarea name="description" id="description"
                                    ...
                                    placeholder="Additional notes about this financial statement..."
            ...
            <div class="modern-form-actions">
                <a href="{{ route('admin.finance-statements.index') }}" class="btn-modern btn-modern-ghost">
                    Cancel
                </a>
