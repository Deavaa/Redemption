...
<div class="modern-page">
    ...
    <div class="modern-page-header">
        <div class="modern-page-header-left">
            ...
            <h1 class="modern-page-title">Finance Statements</h1>
            <p class="modern-page-subtitle">View and manage financial statements and reports</p>
        </div>
        <div class="modern-page-header-right">
            <a href="{{ route('admin.finance-statements.create') }}" class="btn-modern btn-modern-primary">
    ...
    <div class="modern-stats-row">
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                ...
                <span class="modern-stat-label">Total Statements</span>
            </div>
        </div>
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <span class="modern-stat-value">{{ number_format($data->sum('total_income'), 2) }}</span>
                <span class="modern-stat-label">Total Income</span>
            </div>
        </div>
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <span class="modern-stat-value">{{ number_format($data->sum('net_balance'), 2) }}</span>
                <span class="modern-stat-label">Net Balance</span>
            </div>
        </div>
    </div>
    ...
    <div class="modern-card">
        <div class="modern-card-header">
            <div class="modern-card-header-left">
                <h2 class="modern-card-title">All Statements</h2>
                <span class="modern-badge modern-badge-light">{{ $data->total() }} records</span>
            </div>
            <div class="modern-card-header-right">
                <div class="modern-search-box">
                    <i class="fas fa-search"></i>
                    <input type="text" id="statementSearch" placeholder="Search statements..." onkeyup="filterTable()">
                </div>
            </div>
        </div>
        <div class="modern-card-body">
            ...
            <div class="modern-table-wrapper">
                <table class="modern-table" id="statementTable">
                    <thead>
                        <tr>
                            ...
                            <th>Academic Year</th>
                            <th>Period</th>
                            <th class="th-center">Total Income</th>
                            <th class="th-center">Total Expense</th>
                            <th class="th-center">Net Balance</th>
                            <th>Branch</th>
                    ...
                    <tbody>
                        ...
                        <tr class="modern-table-row">
                            ...
                            <td class="td-center">
                                @switch($item->statement_type)
                                    @case('income_statement')
                                        <span class="modern-badge modern-badge-success">Income Statement</span>
                                        @break
                                    @case('balance_sheet')
                                        <span class="modern-badge modern-badge-blue">Balance Sheet</span>
                                        @break
                                    @case('cash_flow')
                                        <span class="modern-badge modern-badge-purple">Cash Flow</span>
                                        @break
                                    @case('trial_balance')
                                        <span class="modern-badge modern-badge-gold">Trial Balance</span>
                                        @break
                                    @default
                                        <span class="modern-badge modern-badge-light">{{ ucfirst(str_replace('_', ' ', $item->statement_type)) }}</span>
                                @endswitch
                            </td>
                            <td>
                                <div class="modern-cell-title">{{ $item->academicYear->name ?? '-' }}</div>
                            </td>
                            <td>
                                <div class="modern-cell-text">
                                    {{ $item->period_from ? $item->period_from->format('M d, Y') : '-' }} -
                                    {{ $item->period_to ? $item->period_to->format('M d, Y') : '-' }}
                                </div>
                            </td>
                            <td class="td-center">
                                <span class="modern-cell-text-green">{{ number_format($item->total_income, 2) }}</span>
                            </td>
                            <td class="td-center">
                                <span class="modern-cell-text-red">{{ number_format($item->total_expense, 2) }}</span>
                            </td>
                            <td class="td-center">
                                @if($item->net_balance >= 0)
                                    <span class="modern-cell-text-green">{{ number_format($item->net_balance, 2) }}</span>
                                @else
                                    <span class="modern-cell-text-red">{{ number_format($item->net_balance, 2) }}</span>
                                @endif
                            </td>
                            <td>
                                <div class="modern-cell-text">{{ $item->branch->name ?? 'All' }}</div>
                            </td>
                            <td class="td-actions">
                                <div class="modern-action-group">
                                    <a href="{{ route('admin.finance-statements.edit', $item->id) }}" class="modern-btn-icon modern-btn-edit" title="Edit">
                                        <i class="fas fa-pen"></i>
                                    </a>
                                    <form method="POST" action="{{ route('admin.finance-statements.destroy', $item->id) }}" style="display:inline" onsubmit="return confirm('Are you sure you want to delete this statement?')">
                                        @csrf @method('DELETE')
            ...
            <div class="modern-empty-state">
                ...
                <p>Get started by creating your first financial statement.</p>
                <a href="{{ route('admin.finance-statements.create') }}" class="btn-modern btn-modern-primary">
...
.modern-badge-purple { background: #f5f3ff; color: #7c3aed; }
