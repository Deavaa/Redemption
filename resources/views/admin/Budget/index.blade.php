...
<div class="modern-page">
    ...
    <div class="modern-page-header">
        <div class="modern-page-header-left">
            ...
            <h1 class="modern-page-title">Budgets</h1>
            <p class="modern-page-subtitle">Manage budget allocations and tracking</p>
        </div>
        <div class="modern-page-header-right">
            <a href="{{ route('admin.budgets.create') }}" class="btn-modern btn-modern-primary">
    ...
    <div class="modern-stats-row">
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                ...
                <span class="modern-stat-label">Total Budgets</span>
            </div>
        </div>
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <span class="modern-stat-value">{{ number_format($data->sum('allocated_amount'), 2) }}</span>
                <span class="modern-stat-label">Total Allocated</span>
            </div>
        </div>
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <span class="modern-stat-value">{{ number_format($data->sum('spent_amount'), 2) }}</span>
                <span class="modern-stat-label">Total Spent</span>
            </div>
        </div>
    </div>
    ...
    <div class="modern-card">
        <div class="modern-card-header">
            <div class="modern-card-header-left">
                <h2 class="modern-card-title">All Budgets</h2>
                <span class="modern-badge modern-badge-light">{{ $data->total() }} records</span>
            </div>
            <div class="modern-card-header-right">
                <div class="modern-search-box">
                    <i class="fas fa-search"></i>
                    <input type="text" id="budgetSearch" placeholder="Search budgets..." onkeyup="filterTable()">
                </div>
            </div>
        </div>
        <div class="modern-card-body">
            ...
            <div class="modern-table-wrapper">
                <table class="modern-table" id="budgetTable">
                    <thead>
                        <tr>
                            <th class="th-narrow">#</th>
                            <th>Category</th>
                            <th>Academic Year</th>
                            <th class="th-center">Allocated</th>
                            <th class="th-center">Spent</th>
                    ...
                    <tbody>
                        ...
                        <tr class="modern-table-row">
                            ...
                            <td>
                                <div class="modern-cell-title">{{ ucfirst($item->category ?? '-') }}</div>
                                @if($item->description)
                                    <div class="modern-cell-sub">{{ \Illuminate\Support\Str::limit($item->description, 40) }}</div>
                                @endif
                            </td>
                            <td>
                            ...
                            <td class="td-center">
                                <span class="modern-cell-amount">{{ number_format($item->allocated_amount, 2) }}</span>
                            </td>
                            <td class="td-center">
                                <span class="modern-cell-text-red">{{ number_format($item->spent_amount, 2) }}</span>
                            </td>
                            <td class="td-center">
                                @php $remaining = $item->allocated_amount - $item->spent_amount; @endphp
                                <span class="modern-cell-text-green">{{ number_format($remaining, 2) }}</span>
                            </td>
                            <td class="td-center">
                                @switch($item->status)
                                    @case('active')
                                        ...
                                        @break
                                    @case('approved')
                                        <span class="modern-badge modern-badge-blue">Approved</span>
                                        @break
                                    @case('draft')
                                        <span class="modern-badge modern-badge-gold">Draft</span>
                                        @break
                                    @case('closed')
                                        <span class="modern-badge modern-badge-light">Closed</span>
                                        @break
                                    @default
                            ...
                            <td class="td-actions">
                                <div class="modern-action-group">
                                    <a href="{{ route('admin.budgets.edit', $item->id) }}" class="modern-btn-icon modern-btn-edit" title="Edit">
                                        <i class="fas fa-pen"></i>
                                    </a>
                                    <form method="POST" action="{{ route('admin.budgets.destroy', $item->id) }}" style="display:inline" onsubmit="return confirm('Are you sure you want to delete this budget?')">
                                        @csrf @method('DELETE')
            ...
            <div class="modern-empty-state">
                ...
                <p>Get started by creating your first budget allocation.</p>
                <a href="{{ route('admin.budgets.create') }}" class="btn-modern btn-modern-primary">
...
.modern-card-header {
    ...
    padding: 1.25rem 1.5rem; border-bottom: 1px solid #f0f0f0; flex-wrap: wrap; gap: 1rem;
}
...
.modern-search-box input {
    ...
    padding: 0.55rem 0.75rem 0.55rem 2.25rem; font-size: 0.875rem;
    width: 220px; transition: all 0.2s; background: #f9fafb; color: #374151;
}
...
.modern-table thead th {
    background: #f9fafb; padding: 0.85rem 1rem; text-align: left;
    font-weight: 600; font-size: 0.78rem; text-transform: uppercase;
    letter-spacing: 0.5px; color: #6b7280; border-bottom: 2px solid #e5e7eb; white-space: nowrap;
}
...
.modern-row-number {
    ...
    background: #f3f4f6; color: #6b7280; font-weight: 600; font-size: 0.8rem;
}
...
.modern-cell-amount { font-weight: 700; color: #059669; font-size: 0.9rem; }
.modern-cell-text-green { color: #059669; font-weight: 600; font-size: 0.88rem; }
.modern-cell-text-red { color: #dc2626; font-weight: 600; font-size: 0.88rem; }
...
.modern-btn-icon {
    ...
    display: inline-flex; align-items: center; justify-content: center;
    cursor: pointer; transition: all 0.2s; font-size: 0.82rem; text-decoration: none;
}
...
.btn-modern {
    ...
    font-size: 0.9rem; text-decoration: none; border: none; cursor: pointer; transition: all 0.25s;
}
