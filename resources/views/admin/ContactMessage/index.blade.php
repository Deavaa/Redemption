    ...
    <div class="modern-stats-row">
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                ...
                <span class="modern-stat-label">Total Messages</span>
            </div>
        </div>
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <span class="modern-stat-value">{{ $data->where('is_read', true)->count() }}</span>
                <span class="modern-stat-label">Read</span>
            </div>
        </div>
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <span class="modern-stat-value">{{ $data->where('is_read', false)->count() }}</span>
    ...
    <div class="modern-card">
        <div class="modern-card-header">
            <div class="modern-card-header-left">
                <h2 class="modern-card-title">All Messages</h2>
                <span class="modern-badge modern-badge-light">{{ $data->total() }} records</span>
            </div>
            <div class="modern-card-header-right">
                <div class="modern-search-box">
                    <i class="fas fa-search"></i>
                    <input type="text" id="contactSearch" placeholder="Search messages..." onkeyup="filterTable()">
                </div>
            </div>
        </div>
        <div class="modern-card-body">
            ...
            <div class="modern-table-wrapper">
                <table class="modern-table" id="contactTable">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        ...
                        <tr class="{{ ! $item->is_read ? 'modern-row-unread' : '' }}">
                            <td class="td-narrow">
                            ...
                            <td>
                                ...
                                @if($item->phone)
                                    <div class="modern-cell-muted">{{ $item->phone }}</div>
                                @endif
                            </td>
                            <td>
                                <div class="modern-cell-text">{{ $item->subject }}</div>
                            </td>
                            <td>
                                <div class="modern-cell-text" style="max-width:250px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">{{ Str::limit($item->message, 60) }}</div>
                            </td>
                            <td class="td-center">
                                @if($item->is_read)
                            ...
                            <td class="td-actions">
                                <div class="modern-action-group">
                                    <a href="{{ route('admin.contact-messages.show', $item->id) }}" class="modern-btn-icon modern-btn-view" title="View">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="{{ route('admin.contact-messages.edit', $item->id) }}" class="modern-btn-icon modern-btn-edit" title="Edit">
                                        <i class="fas fa-pen"></i>
                                    </a>
                                    <form method="POST" action="{{ route('admin.contact-messages.destroy', $item->id) }}" style="display:inline" onsubmit="return confirm('Are you sure you want to delete this message?')">
                                        @csrf @method('DELETE')
            ...
            <div class="modern-empty-state">
                ...
                <p>Messages from the website contact form will appear here.</p>
            </div>
            @endif
        </div>
    </div>
...
.modern-row-unread { background: #fffbf0; }
.modern-row-unread:hover { background: #fff8e8; }
