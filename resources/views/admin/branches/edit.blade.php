...
<h4 class="mb-0"><i class="fas fa-map-marker-alt me-2 text-danger"></i>Edit Branch - {{$branch->name}}</h4>
...
<input type="number" name="order" class="form-control" min="0" value="{{old('order',$branch->order)}}"></div>
</div>
</div>
...
<input type="tel" name="phone" class="form-control" value="{{old('phone',$branch->phone)}}"></div>
</div>
<div class="col-md-6">
<label class="form-label fw-semibold">Email</label>
<div class="input-group"><span class="input-group-text"><i class="fas fa-envelope"></i></span>
<input type="email" name="email" class="form-control" value="{{old('email',$branch->email)}}"></div>
</div>
</div>
<div class="row g-3 mt-1">
<div class="col-12">
...
<option value="{{$t->id}}" {{old('principal_id',$branch->principal_id)==$t->id?'selected':''}}>{{$t->first_name}} {{$t->last_name}} @if($t->department)({{$t->department}})@endif</option>
@endforeach
</select>
...
<small class="text-muted">Select the branch principal from existing teachers or add a new one</small>
</div>
</div>
<div class="row g-3 mt-1">
<div class="col-12">
...
<textarea name="address" class="form-control" rows="2">{{old('address',$branch->address)}}</textarea></div>
</div>
</div>
</div>
</div>
...
<input type="text" name="gps_lat" class="form-control" id="latInput" value="{{old('gps_lat',$branch->gps_lat)}}" oninput="updateMapPreview()"></div>
</div>
<div class="col-md-6">
...
<input type="checkbox" class="form-check-input" name="is_active" id="isActive" value="1" {{old('is_active',$branch->is_active)?'checked':''}}>
...
<div class="col-md-6">
<div class="form-check form-switch mt-2">
<input type="checkbox" class="form-check-input" name="is_headquarters" id="isHQ" value="1" {{old('is_headquarters',$branch->is_headquarters)?'checked':''}}>
...
<div class="mb-4">
<button type="submit" class="btn btn-primary px-4"><i class="fas fa-save me-2"></i>Update Branch</button>
...
<h6>Editing Branch</h6>
<p class="text-muted small mb-0">Modify branch details, principal, and map location.</p>
</div>
</div>
</div>
</div>
...
<button type="submit" class="btn btn-primary"><i class="fas fa-save me-1"></i>Save & Select</button>
</div>
</form>
</div>
</div>
...
updateMapPreview();
 ...
 $('#addTeacherForm').on('submit',function(e){
    ...
    $.ajax({
        ...
        complete:function(){btn.prop('disabled',false).html('<i class="fas fa-save me-1"></i>Save & Select');}
    });
});
</script>
@endsection
