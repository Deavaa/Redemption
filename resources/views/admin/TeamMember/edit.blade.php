...
.tm-current-photo{margin-bottom:12px;border-radius:12px;overflow:hidden;border:2px solid #e2e8f0;}
.tm-current-photo img{width:100%;max-height:150px;object-fit:cover;display:block;}
.tm-current-photo-label{padding:8px 12px;background:#f8fafc;font-size:0.75rem;color:#6b7280;border-top:1px solid #e2e8f0;display:flex;align-items:center;justify-content:space-between;}
    ...
    <div class="tm-card">
        <form method="POST" action="{{ route('admin.team-members.update', $item->id) }}" enctype="multipart/form-data">
            ...
            <div class="tm-section">
                <div class="tm-section-head">
                    ...
                    <div>
                        ...
                        <p class="tm-section-desc">Upload a new photo — auto-compressed in your browser before upload</p>
                    </div>
                </div>
                <div class="tm-section-body">
                    @php
                        ...
                        if ($item->photo) {
                            $basename = basename($item->photo);
                            if (file_exists(public_path('team-photos/' . $basename))) {
                                $existingPhotoUrl = asset('team-photos/' . $basename);
                            } elseif (file_exists(public_path('storage/' . $item->photo))) {
                                $existingPhotoUrl = asset('storage/' . $item->photo);
                            }
                        }
                    @endphp
                    @if($existingPhotoUrl)
                    <div class="tm-current-photo">
                        <img src="{{ $existingPhotoUrl }}" alt="Current photo">
                        <div class="tm-current-photo-label">
                            <span><i class="fas fa-check-circle" style="color:#059669;"></i> Current photo</span>
                            <span style="font-size:0.7rem;color:#9ca3af;">{{ basename($item->photo) }}</span>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="tm-photo-zone" id="photoZone">
                        ...
                        <p class="tm-photo-title">Click to upload new photo or drag & drop</p>
                    ...
                    <div class="tm-photo-preview" id="photoPreview">
                        <img src="" alt="New preview" id="previewImg">
                ...
                <div class="tm-section-body">
                    <div class="tm-grid">
                        <div class="tm-field">
                            ...
                            <input type="text" name="name" class="tm-input" value="{{ old('name', $item->name) }}" placeholder="e.g. John Doe" required>
                        </div>
                        <div class="tm-field">
                            <label class="tm-label">Designation <span class="req">*</span></label>
                            <input type="text" name="designation" class="tm-input" value="{{ old('designation', $item->designation) }}" placeholder="e.g. Principal" required>
                        </div>
                        <div class="tm-field">
                            <label class="tm-label">Department <small>(optional)</small></label>
                            <input type="text" name="department" class="tm-input" value="{{ old('department', $item->department) }}" placeholder="e.g. Science">
                        </div>
                        <div class="tm-field">
                            <label class="tm-label">Qualification <small>(optional)</small></label>
                            <input type="text" name="qualification" class="tm-input" value="{{ old('qualification', $item->qualification) }}" placeholder="e.g. M.Ed, PhD">
                        </div>
                        <div class="tm-field">
                            <label class="tm-label">Experience <small>(optional)</small></label>
                            <input type="text" name="experience" class="tm-input" value="{{ old('experience', $item->experience) }}" placeholder="e.g. 10 years">
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="tm-section-body">
                    <div class="tm-grid">
                        <div class="tm-field">
                            ...
                            <input type="email" name="email" class="tm-input" value="{{ old('email', $item->email) }}" placeholder="e.g. john@school.com">
                        </div>
                        <div class="tm-field">
                            <label class="tm-label">Phone <small>(optional)</small></label>
                            <input type="text" name="phone" class="tm-input" value="{{ old('phone', $item->phone) }}" placeholder="e.g. +251 911 234 567">
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="tm-section-body">
                    <div class="tm-grid">
                        <div class="tm-field tm-span2">
                            ...
                            <textarea name="bio" class="tm-input tm-textarea" rows="4" placeholder="Brief biography or description...">{{ old('bio', $item->bio) }}</textarea>
                        </div>
                        <div class="tm-field">
                            <label class="tm-label">Sort Order <small>(optional)</small></label>
                            <input type="number" name="sort_order" class="tm-input" value="{{ old('sort_order', $item->sort_order) }}" min="0" placeholder="0">
                        </div>
                        <div class="tm-field">
            ...
            <div class="tm-actions">
                ...
                <button type="submit" class="tm-btn tm-btn-primary"><i class="fas fa-save"></i>Save Changes</button>
            </div>
        </form>
    </div>
</div>
