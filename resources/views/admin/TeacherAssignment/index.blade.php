    ...
    <div class="modern-stats-row">
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                ...
                <div class="modern-stat-label">Total Assignments</div>
            </div>
        </div>
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <div class="modern-stat-value">{{ $data->pluck('teacher_id')->unique()->count() }}</div>
                <div class="modern-stat-label">Active Teachers</div>
            </div>
        </div>
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <div class="modern-stat-value">{{ $data->pluck('subject_id')->unique()->count() }}</div>
                <div class="modern-stat-label">Subjects Covered</div>
            </div>
        </div>
    </div>
    ...
    <div class="modern-card">
        <div class="modern-card-header">
            <div class="modern-card-header-left">
                <h2 class="modern-card-title">All Assignments</h2>
            </div>
            <div class="modern-card-header-right">
                <div class="modern-search-box">
                    <i class="fas fa-search"></i>
                    <input type="text" id="assignmentSearch" placeholder="Search assignments..." onkeyup="filterTable()">
                </div>
            </div>
        </div>
        <div class="modern-card-body">
            ...
            <div class="modern-table-wrapper">
                <table class="modern-table" id="assignmentTable">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @if($item->teacher)
                                    <div class="modern-cell-title">{{ $item->teacher->name ?? '-' }}</div>
                                    <div class="modern-cell-sub">ID: {{ $item->teacher_id }}</div>
                                @else
                                    <span class="modern-cell-muted">ID: {{ $item->teacher_id }}</span>
                                @endif
                            </td>
                            <td>
                                @if($item->classRoom)
                                ...
                                @else
                                    <span class="modern-cell-muted">ID: {{ $item->class_id }}</span>
                                @endif
                            </td>
                            <td>
                                @if($item->section)
                                ...
                                @else
                                    <span class="modern-cell-muted">ID: {{ $item->section_id }}</span>
                                @endif
                            </td>
                            <td>
                                @if($item->subject)
                                ...
                                @else
                                    <span class="modern-cell-muted">ID: {{ $item->subject_id }}</span>
                                @endif
                            </td>
                            <td>
                                ...
                                @else
                                    <span class="modern-cell-muted">ID: {{ $item->academic_year_id }}</span>
                                @endif
                            </td>
                            <td>
                                <div class="modern-action-group">
                                    <a href="{{ route('admin.teacher-assignments.show', $item->id) }}" class="modern-btn-view" title="View">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="{{ route('admin.teacher-assignments.edit', $item->id) }}" class="modern-btn-edit" title="Edit">
                                        <i class="fas fa-pen"></i>
                                    </a>
                                    <form method="POST" action="{{ route('admin.teacher-assignments.destroy', $item->id) }}" style="display:inline" onsubmit="return confirm('Are you sure you want to delete this assignment?')">
                                        @csrf
                                        @method('DELETE')
            ...
            <div class="modern-empty-state">
                ...
                <p>Start by creating your first teacher assignment.</p>
                <a href="{{ route('admin.teacher-assignments.create') }}" class="btn-modern btn-modern-primary">
                    <i class="fas fa-plus"></i> Add Assignment
                </a>
            </div>
...
@push('scripts')
<script>
function filterTable(){const e=document.getElementById('assignmentSearch').value.toLowerCase(),t=document.querySelectorAll('#assignmentTable tbody tr');t.forEach(t=>{t.textContent.toLowerCase().includes(e)?t.style.display='':t.style.display='none'})}
</script>
@endpush
@endsection
