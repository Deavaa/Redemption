        ...
        <div class="modern-page-header-left">
            <nav aria-label="breadcrumb" class="modern-breadcrumb">
                <ol>
                    ...
                    <li class="active">Mark Entries</li>
                </ol>
            </nav>
            <h1 class="modern-page-title">Mark Entries</h1>
            <p class="modern-page-subtitle">Manage student marks and assessment scores</p>
        </div>
        <div class="modern-page-header-right">
            <a href="{{ route('admin.mark-entries.create') }}" class="btn-modern btn-modern-primary">
    ...
    <div class="modern-stats-row">
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <span class="modern-stat-value">{{ $data->total() ?? 0 }}</span>
                <span class="modern-stat-label">Total Entries</span>
            </div>
        </div>
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <span class="modern-stat-value">{{ $data->count() > 0 ? $data->pluck('student_id')->unique()->count() : 0 }}</span>
        ...
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <span class="modern-stat-value">{{ $data->count() > 0 ? $data->pluck('subject_id')->unique()->count() : 0 }}</span>
                <span class="modern-stat-label">Subjects</span>
            </div>
        </div>
    </div>

    {{-- Mark Entries Table Card --}}
    <div class="modern-card">
        <div class="modern-card-header">
            <div class="modern-card-header-left">
                <h2 class="modern-card-title">All Mark Entries</h2>
                <span class="modern-badge modern-badge-light">{{ $data->total() ?? 0 }} records</span>
            </div>
            <div class="modern-card-header-right">
                <div class="modern-search-box">
                    <i class="fas fa-search"></i>
                    <input type="text" id="markEntrySearch" placeholder="Search entries..." onkeyup="filterTable()">
                </div>
            </div>
        </div>
        <div class="modern-card-body">
            ...
            <div class="modern-table-wrapper">
                <table class="modern-table" id="markEntryTable">
                    <thead>
                        <tr>
                            ...
                            <th>Exam</th>
                            <th>Subject</th>
                            <th class="th-center">Marks Obtained</th>
                            <th class="th-center">Max Marks</th>
                    ...
                    <tbody>
                        ...
                        <tr class="modern-table-row">
                            ...
                            <td>
                                <div class="modern-cell-title">{{ $item->student_id ?? '-' }}</div>
                            </td>
                            <td>
                                <div class="modern-cell-text">{{ $item->exam_id ?? '-' }}</div>
                            </td>
                            <td>
                                <div class="modern-cell-text">{{ $item->subject_id ?? '-' }}</div>
                            </td>
                            <td class="td-center">
                                @if($item->marks_obtained !== null)
                                    <span class="modern-badge modern-badge-blue">{{ $item->marks_obtained }}</span>
                                @else
                                    <span class="modern-cell-muted">-</span>
                                @endif
                            </td>
                            <td class="td-center">
                                <span class="modern-cell-text">{{ $item->max_marks ?? '-' }}</span>
                            </td>
                            <td class="td-center">
                                @if($item->grade)
                                    @php
                                        $gradeClass = 'modern-badge-warning';
                                        if (in_array($item->grade, ['A+', 'A', 'A-'])) $gradeClass = 'modern-badge-success';
                                        elseif (in_array($item->grade, ['B+', 'B', 'B-'])) $gradeClass = 'modern-badge-blue';
                                        elseif (in_array($item->grade, ['C+', 'C', 'C-'])) $gradeClass = 'modern-badge-warning';
                                        elseif (in_array($item->grade, ['D'])) $gradeClass = 'modern-badge-orange';
                                        else $gradeClass = 'modern-badge-danger';
                                    @endphp
                                    <span class="modern-badge {{ $gradeClass }}">{{ $item->grade }}</span>
                                @else
                                    <span class="modern-cell-muted">-</span>
                                @endif
                            </td>
                            <td class="td-actions">
                                <div class="modern-action-group">
                                    <a href="{{ route('admin.mark-entries.show', $item->id) }}" class="modern-btn-icon modern-btn-view" title="View">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="{{ route('admin.mark-entries.edit', $item->id) }}" class="modern-btn-icon modern-btn-edit" title="Edit">
                                        <i class="fas fa-pen"></i>
                                    </a>
                                    <form method="POST" action="{{ route('admin.mark-entries.destroy', $item->id) }}" style="display:inline" onsubmit="return confirm('Are you sure you want to delete this mark entry?')">
                                        @csrf @method('DELETE')
            ...
            <div class="modern-empty-state">
                ...
                <h3>No Mark Entries Yet</h3>
                <p>Get started by adding your first mark entry.</p>
                <a href="{{ route('admin.mark-entries.create') }}" class="btn-modern btn-modern-primary">
...
function filterTable() {
    const input = document.getElementById('markEntrySearch');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('markEntryTable');
