    ...
    <div class="modern-stats-row">
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                ...
                <span class="modern-stat-label">Total Audits</span>
            </div>
        </div>
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <span class="modern-stat-value">{{ $data->where('status', 'Completed')->count() + $data->where('status', 'completed')->count() }}</span>
        ...
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <span class="modern-stat-value">{{ $data->where('status', '!=', 'Completed')->where('status', '!=', 'completed')->count() }}</span>
                <span class="modern-stat-label">Pending / In Progress</span>
            </div>
        </div>
    </div>
    ...
    <div class="modern-card">
        <div class="modern-card-header">
            <div class="modern-card-header-left">
                <h2 class="modern-card-title">All Audit Records</h2>
                <span class="modern-badge modern-badge-light">{{ $data->total() }} records</span>
            </div>
            <div class="modern-card-header-right">
                <div class="modern-search-box">
                    <i class="fas fa-search"></i>
                    <input type="text" id="auditSearch" placeholder="Search audits..." onkeyup="filterTable()">
                </div>
            </div>
        </div>
        <div class="modern-card-body">
                ...
                <table class="modern-table" id="auditTable">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="modern-cell-title">{{ $item->auditor_name ?? '-' }}</div>
                                @if($item->academicYear)
                                    <div class="modern-cell-sub">{{ $item->academicYear->name }}</div>
                                @endif
                            </td>
                            <td>
                                <div class="modern-cell-text">
                                    @if($item->audit_date)
                                        {{ $item->audit_date->format('M d, Y') }}
                                    @else
                                        -
                                    @endif
                                </div>
                            ...
                            <td>
                                <div class="modern-cell-text">{{ $item->findings ?? '-' }}</div>
                            </td>
                            <td>
                                <div class="modern-cell-text">{{ $item->recommendations ?? '-' }}</div>
                            </td>
                            <td class="td-center">
                                @if(strtolower($item->status ?? '') === 'completed')
                                    <span class="modern-badge modern-badge-success">Completed</span>
                                @elseif(strtolower($item->status ?? '') === 'in progress')
                                ...
                                @elseif(strtolower($item->status ?? '') === 'critical')
                                    <span class="modern-badge modern-badge-danger">Critical</span>
                                @else
                                    <span class="modern-badge modern-badge-light">{{ $item->status ?? '-' }}</span>
                                @endif
                            </td>
                            <td class="td-actions">
                                <div class="modern-action-group">
                                    <a href="{{ route('admin.audits.show', $item->id) }}" class="modern-btn-icon modern-btn-view" title="View">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="{{ route('admin.audits.edit', $item->id) }}" class="modern-btn-icon modern-btn-edit" title="Edit">
                                        <i class="fas fa-pen"></i>
                                    </a>
                                    <form method="POST" action="{{ route('admin.audits.destroy', $item->id) }}" style="display:inline" onsubmit="return confirm('Are you sure you want to delete this audit record?')">
                                        @csrf @method('DELETE')
            ...
            <div class="modern-empty-state">
                ...
                <p>Start by adding your first audit record.</p>
                <a href="{{ route('admin.audits.create') }}" class="btn-modern btn-modern-primary">
