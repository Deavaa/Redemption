...
@section('title', 'Edit Teacher Efficiency Assessment')
...
<div class="modern-page">
    <div class="modern-page-header">
        <div class="modern-page-header-left">
            <nav aria-label="breadcrumb" class="modern-breadcrumb">
                <ol><li><a href="{{ route('admin.dashboard') }}"><i class="fas fa-home"></i></a></li><li><a href="{{ route('admin.teacher-efficiency.index') }}">Teacher Efficiency</a></li><li class="active">Edit Draft</li></ol>
            </nav>
        </div>
        <div class="modern-page-header-right">
            <a href="{{ route('admin.teacher-efficiency.show', $teacherEfficiencyAssessment->id) }}" class="btn-modern btn-modern-outline"><i class="fas fa-eye"></i><span>View</span></a>
    ...
    <div class="modern-card">
        <form method="POST" action="{{ route('admin.teacher-efficiency.update', $teacherEfficiencyAssessment->id) }}" id="assessmentForm">
            @csrf @method('PUT')
                    ...
                    <div class="modern-form-grid">
                        <div class="modern-form-group">
                            ...
                            <select name="branch_id" id="branchSelect" class="modern-input modern-select" required>
                                ...
                                <option value="{{ $b->id }}" {{ old('branch_id', $teacherEfficiencyAssessment->branch_id) == $b->id ? 'selected' : '' }}>{{ $b->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="modern-form-group">
                            ...
                            <select name="teacher_id" id="teacherSelect" class="modern-input modern-select" required>
                                ...
                                <option value="{{ $t->id }}" data-branch="{{ $t->branch_id }}" {{ old('teacher_id', $teacherEfficiencyAssessment->teacher_id) == $t->id ? 'selected' : '' }}>{{ $t->full_name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="modern-form-group">
                            ...
                            <select name="academic_year_id" class="modern-input modern-select" required>
                                ...
                                <option value="{{ $ay->id }}" {{ old('academic_year_id', $teacherEfficiencyAssessment->academic_year_id) == $ay->id ? 'selected' : '' }}>{{ $ay->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="modern-form-group">
                            ...
                            <select name="term_id" class="modern-input modern-select" required>
                                ...
                                <option value="{{ $t->id }}" {{ old('term_id', $teacherEfficiencyAssessment->term_id) == $t->id ? 'selected' : '' }}>{{ $t->name }}</option>
                                @endforeach
                            </select>
                ...
                <div class="modern-form-section-body" style="padding:1.25rem 2rem 1.75rem;">
                    ...
                    <div class="eq-criteria-row">
                        ...
                        <div class="eq-criteria-radios">
                            ...
                            <label class="eq-radio-label" title="{{ $scaleLabels[$i] }}">
                                <input type="radio" name="{{ $field }}" value="{{ $i }}" class="eq-criteria-input" {{ old($field, $teacherEfficiencyAssessment->$field) == $i ? 'checked' : '' }} required>
                    ...
                    <div class="eq-score-display" id="liveScoreDisplay">
                        <div class="eq-score-left">
                            ...
                            <span class="eq-score-value" id="liveScore">{{ $teacherEfficiencyAssessment->overall_score }}</span>
                            <span class="eq-score-max">/100</span>
                        </div>
                        <div class="eq-score-right">
                            <span class="eq-grade-badge" id="liveGrade">{{ $teacherEfficiencyAssessment->grade_label }}</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="modern-form-section-body">
                    <div class="modern-form-grid">
                        <div class="modern-form-group">
                            ...
                            <textarea name="strengths" class="modern-input modern-textarea" rows="3" placeholder="What the teacher does well...">{{ old('strengths', $teacherEfficiencyAssessment->strengths) }}</textarea>
                        </div>
                        <div class="modern-form-group">
                            <label class="modern-form-label">Areas for Improvement</label>
                            <textarea name="areas_for_improvement" class="modern-input modern-textarea" rows="3" placeholder="Where the teacher can improve...">{{ old('areas_for_improvement', $teacherEfficiencyAssessment->areas_for_improvement) }}</textarea>
                        </div>
                        <div class="modern-form-group">
                            <label class="modern-form-label">Action Plan</label>
                            <textarea name="action_plan" class="modern-input modern-textarea" rows="3" placeholder="Recommended actions for improvement...">{{ old('action_plan', $teacherEfficiencyAssessment->action_plan) }}</textarea>
                        </div>
                        <div class="modern-form-group">
                            <label class="modern-form-label">General Comments</label>
                            <textarea name="comments" class="modern-input modern-textarea" rows="3" placeholder="Any additional notes...">{{ old('comments', $teacherEfficiencyAssessment->comments) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
