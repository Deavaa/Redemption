    ...
    <div class="modern-card">
        <div class="modern-card-header">
            <div class="modern-card-header-left">
                <h2 class="modern-card-title"><i class="fas fa-pen" style="color:#d97706;margin-right:8px;"></i>Edit Video</h2>
            </div>
        </div>
        <div class="modern-card-body" style="padding:1.5rem;">
            ...
            <form method="POST" action="{{ route('admin.video-library.update', $video->id) }}" id="videoForm">
                @csrf @method('PUT')
                ...
                <div class="form-group mb-4">
                    ...
                    <div class="video-type-toggle">
                        <label class="video-type-option {{ $video->video_type === 'single' ? 'active' : '' }}" id="typeSingle">
                            <input type="radio" name="video_type" value="single" {{ $video->video_type === 'single' ? 'checked' : '' }} style="display:none;">
                            <i class="fas fa-video"></i>
                            <span>Single Video</span>
                        </label>
                        <label class="video-type-option {{ $video->video_type === 'channel' ? 'active' : '' }}" id="typeChannel">
                            <input type="radio" name="video_type" value="channel" {{ $video->video_type === 'channel' ? 'checked' : '' }} style="display:none;">
                ...
                <div class="row">
                    <div class="col-lg-8">
                        ...
                        <div class="form-group mb-3">
                            <label class="form-label" style="font-weight:600;color:#374151;font-size:0.875rem;">
                                <span id="urlLabel">{{ $video->video_type === 'channel' ? 'YouTube Channel URL' : 'YouTube Video URL' }}</span> <span class="text-danger">*</span>
                            </label>
                            ...
                            <small class="text-muted" style="font-size:0.75rem;" id="urlHint">
                                ...
                                Paste a YouTube channel link (e.g., youtube.com/@channelname)
                                @else
                            ...
                            <div id="videoPreview" class="mt-2">
                                <div class="video-preview-container">
                                    <iframe id="previewIframe" width="100%" height="280" frameborder="0" allowfullscreen
                                        src="{{ $video->getEmbedUrl() }}" style="border-radius:10px;"></iframe>
                                </div>
                            </div>
                            @endif
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="title" value="{{ old('title', $video->title) }}" class="form-control" style="border-radius:10px;" placeholder="Enter video title" required maxlength="500">
                        </div>

                        {{-- Description --}}
                        <div class="form-group mb-3">
                            <label class="form-label" style="font-weight:600;color:#374151;font-size:0.875rem;">Description</label>
                            <textarea name="description" class="form-control" style="border-radius:10px;min-height:100px;" placeholder="Brief description of the video content..." maxlength="5000">{{ old('description', $video->description) }}</textarea>
                        </div>

                        {{-- Channel Info --}}
                        <div class="row" id="channelFields" style="{{ $video->video_type === 'channel' ? '' : 'display:none;' }}">
                            <div class="col-md-6 mb-3">
                                <label class="form-label" style="font-weight:600;color:#374151;font-size:0.875rem;">Channel Name</label>
                                <input type="text" name="channel_name" value="{{ old('channel_name', $video->channel_name) }}" class="form-control" style="border-radius:10px;" placeholder="e.g., Khan Academy">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label" style="font-weight:600;color:#374151;font-size:0.875rem;">Channel URL</label>
                                <input type="url" name="channel_url" value="{{ old('channel_url', $video->channel_url) }}" class="form-control" style="border-radius:10px;" placeholder="https://www.youtube.com/@channel">
                            </div>
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <div class="row g-2">
                                <div class="col-4">
                                    <input type="number" name="duration_minutes" id="durationMin" class="form-control" style="border-radius:10px;" placeholder="Min" min="0" value="{{ floor(($video->duration_seconds ?? 0) / 60) }}">
                                </div>
                                <div class="col-4">
                                    <input type="number" name="duration_seconds_input" id="durationSec" class="form-control" style="border-radius:10px;" placeholder="Sec" min="0" max="59" value="{{ ($video->duration_seconds ?? 0) % 60 }}">
                                </div>
                                <div class="col-4 d-flex align-items-center">
                                    <input type="hidden" name="duration_seconds" id="durationTotal" value="{{ $video->duration_seconds }}">
                    ...
                    <div class="col-lg-4">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="category" value="{{ old('category', $video->category) }}" class="form-control" style="border-radius:10px;" list="categoryList" placeholder="e.g., Mathematics, Science">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="branch_id" class="form-select" style="border-radius:10px;">
                                ...
                                <option value="{{ $branch->id }}" {{ old('branch_id', $video->branch_id) == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="access_level" class="form-select" style="border-radius:10px;" required>
                                <option value="all" {{ old('access_level', $video->access_level) === 'all' ? 'selected' : '' }}>Everyone</option>
                                <option value="teacher" {{ old('access_level', $video->access_level) === 'teacher' ? 'selected' : '' }}>Teachers Only</option>
                                <option value="student" {{ old('access_level', $video->access_level) === 'student' ? 'selected' : '' }}>Students Only</option>
                                <option value="staff" {{ old('access_level', $video->access_level) === 'staff' ? 'selected' : '' }}>Staff Only</option>
                                <option value="admin" {{ old('access_level', $video->access_level) === 'admin' ? 'selected' : '' }}>Admin Only</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <div class="form-check form-switch" style="padding-left:2.5rem;">
                                <input type="checkbox" name="is_active" value="1" class="form-check-input" style="width:3rem;height:1.5rem;" id="isActive"
                                    {{ old('is_active', $video->is_active) ? 'checked' : '' }}>
                                <label class="form-check-label ms-2" for="isActive" style="font-weight:600;color:{{ old('is_active', $video->is_active) ? '#059669' : '#9ca3af' }};" id="activeLabel">
                                    {{ old('is_active', $video->is_active) ? 'Active' : 'Inactive' }}
                                </label>
                            </div>
                        </div>
                        ...
                        <div class="form-group mb-4">
                            ...
                            <div class="form-check form-switch" style="padding-left:2.5rem;">
                                <input type="checkbox" name="show_on_website" value="1" class="form-check-input" style="width:3rem;height:1.5rem;" id="showOnWebsite"
                                    {{ old('show_on_website', $video->show_on_website) ? 'checked' : '' }}>
                                <label class="form-check-label ms-2" for="showOnWebsite" style="font-weight:600;color:{{ old('show_on_website', $video->show_on_website) ? '#2563eb' : '#9ca3af' }};" id="websiteLabel">
                                    {{ old('show_on_website', $video->show_on_website) ? 'Show on Website' : 'Admin Only' }}
                                </label>
                            </div>
                            <small class="text-muted" style="font-size:0.72rem;">When enabled, this video will appear on the public website gallery page.</small>
                        </div>
                    </div>
                </div>
