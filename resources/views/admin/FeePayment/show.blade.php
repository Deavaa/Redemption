        ...
        <div class="modern-page-header-left">
            <nav class="modern-breadcrumb">
                <ol>
                    ...
                    <li><a href="{{ route('admin.fee-payments.index') }}">Finance</a></li>
                    <li class="active">Details</li>
                </ol>
            </nav>
        </div>
        <div class="modern-page-header-right">
            <a href="{{ route('admin.fee-payments.index') }}" class="btn-modern btn-modern-outline"><i class="fas fa-arrow-left"></i><span>Back to List</span></a>
            <a href="{{ route('admin.fee-payments.edit', $item->id) }}" class="btn-modern btn-modern-primary"><i class="fas fa-pen"></i><span>Edit</span></a>
        </div>
    </div>
    ...
    <div class="modern-detail-grid">
        <div class="modern-card modern-detail-main">
            <div class="modern-detail-hero">
                <div class="modern-detail-hero-icon"><i class="fas fa-credit-card"></i></div>
                <div class="modern-detail-hero-info">
                    <h2 class="modern-detail-hero-title">{{ $item->receipt_number ?? 'Payment #' . $item->id }}</h2>
                    <div class="modern-detail-hero-badges">
                        @switch($item->status)
                            @case('paid')
                                <span class="modern-badge modern-badge-success"><i class="fas fa-check-circle"></i> Paid</span>
                                @break
                            @case('partial')
                                <span class="modern-badge modern-badge-gold"><i class="fas fa-adjust"></i> Partial</span>
                                @break
                            @case('pending')
                                <span class="modern-badge modern-badge-light"><i class="fas fa-clock"></i> Pending</span>
                                @break
                            @case('overdue')
                                <span class="modern-badge modern-badge-danger"><i class="fas fa-exclamation-circle"></i> Overdue</span>
                                @break
                            @default
                                <span class="modern-badge modern-badge-light">{{ ucfirst($item->status) }}</span>
                        @endswitch
                        @if($item->payment_method)
                            @switch($item->payment_method)
                                @case('cash')
                                    <span class="modern-badge modern-badge-success"><i class="fas fa-money-bill-wave"></i> Cash</span>
                                    @break
                                @case('bank')
                                    <span class="modern-badge modern-badge-info"><i class="fas fa-university"></i> Bank</span>
                                    @break
                                @case('mobile')
                                    <span class="modern-badge modern-badge-gold"><i class="fas fa-mobile-alt"></i> Mobile</span>
                                    @break
                                @case('cheque')
                                    <span class="modern-badge modern-badge-warning"><i class="fas fa-money-check"></i> Cheque</span>
                                    @break
                                @case('online')
                                    <span class="modern-badge modern-badge-info"><i class="fas fa-globe"></i> Online</span>
                                    @break
                                @default
                                    <span class="modern-badge modern-badge-light">{{ ucfirst($item->payment_method) }}</span>
                            @endswitch
                        @endif
                    </div>
                </div>
            ...
            <div class="modern-detail-body">
                <div class="modern-detail-row">
                    <div class="modern-detail-label"><i class="fas fa-hashtag"></i> ID</div>
                    <div class="modern-detail-value">{{ $item->id }}</div>
                </div>
                <div class="modern-detail-row">
                    <div class="modern-detail-label"><i class="fas fa-receipt"></i> Receipt Number</div>
                    <div class="modern-detail-value">{{ $item->receipt_number ?? '-' }}</div>
                </div>
                <div class="modern-detail-row">
                ...
                <div class="modern-detail-row">
                    <div class="modern-detail-label"><i class="fas fa-file-invoice"></i> Fee Type</div>
                    <div class="modern-detail-value">
                        @if($item->fee)
                            {{ ucfirst($item->fee->fee_type ?? '-') }}
                        @else
                            <span class="modern-muted">-</span>
                        @endif
                    </div>
                ...
                <div class="modern-detail-row">
                    <div class="modern-detail-label"><i class="fas fa-money-bill-wave"></i> Amount Paid</div>
                    <div class="modern-detail-value modern-detail-amount">{{ number_format($item->amount_paid, 2) }}</div>
                </div>
                <div class="modern-detail-row">
                    <div class="modern-detail-label"><i class="fas fa-calendar-day"></i> Payment Date</div>
                    <div class="modern-detail-value">{{ $item->payment_date ? $item->payment_date->format('M d, Y') : '-' }}</div>
                </div>
                <div class="modern-detail-row">
                    <div class="modern-detail-label"><i class="fas fa-credit-card"></i> Payment Method</div>
                    <div class="modern-detail-value">
                        @switch($item->payment_method)
                            @case('cash')
                                <span class="modern-badge modern-badge-success">Cash</span>
                                @break
                            @case('bank')
                                <span class="modern-badge modern-badge-info">Bank Transfer</span>
                                @break
                            @case('mobile')
                                <span class="modern-badge modern-badge-gold">Mobile Money</span>
                                @break
                            @case('cheque')
                                <span class="modern-badge modern-badge-warning">Cheque</span>
                                @break
                            @case('online')
                                <span class="modern-badge modern-badge-info">Online Payment</span>
                                @break
                            @default
                                <span class="modern-badge modern-badge-light">{{ ucfirst($item->payment_method ?? '-') }}</span>
                        @endswitch
                ...
                <div class="modern-detail-row">
                    <div class="modern-detail-label"><i class="fas fa-exchange-alt"></i> Transaction ID</div>
                    <div class="modern-detail-value">{{ $item->transaction_id ?? '-' }}</div>
                </div>
                <div class="modern-detail-row">
                    <div class="modern-detail-label"><i class="fas fa-info-circle"></i> Status</div>
                    <div class="modern-detail-value">
                        @switch($item->status)
                            @case('paid')
                                <span class="modern-badge modern-badge-success"><i class="fas fa-check-circle"></i> Paid</span>
                            ...
                            @case('overdue')
                                ...
                                @break
                            @default
                                <span class="modern-badge modern-badge-light">{{ ucfirst($item->status ?? 'N/A') }}</span>
                        @endswitch
                    </div>
                </div>
            </div>
        ...
        <div class="modern-detail-sidebar">
            <div class="modern-card">
                ...
                <div class="modern-quick-actions">
                    <a href="{{ route('admin.fee-payments.edit', $item->id) }}" class="modern-quick-action">
                        <i class="fas fa-pen"></i>
                        <span>Edit Payment</span>
                    </a>
                    <a href="{{ route('admin.fee-payments.index') }}" class="modern-quick-action">
                        <i class="fas fa-list"></i>
                        <span>All Payments</span>
                    </a>
                    <form method="POST" action="{{ route('admin.fee-payments.destroy', $item->id) }}" onsubmit="return confirm('Are you sure you want to delete this payment record?')">
                        @csrf @method('DELETE')
...
.modern-detail-hero { display: flex; align-items: center; gap: 1.25rem; padding: 1.75rem 2rem; background: linear-gradient(135deg, #f8f9ff 0%, #eef2ff 100%); border-bottom: 1px solid #e5e8ff; }
...
.modern-detail-amount { font-weight: 700; color: #059669; font-size: 1rem; }
