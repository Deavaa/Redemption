...
<div class="modern-page">
        ...
        <div class="modern-page-header-left">
            <nav aria-label="breadcrumb" class="modern-breadcrumb">
                <ol>
                    ...
                    <li><a href="{{ route('admin.fee-payments.index') }}">Fee Payments</a></li>
                    <li class="active">Edit</li>
                </ol>
            </nav>
            <h1 class="modern-page-title">Edit Payment</h1>
            <p class="modern-page-subtitle">Update payment <strong>{{ $item->receipt_number ?? '#' . $item->id }}</strong></p>
        </div>
    ...
    <div class="modern-card">
        <form method="POST" action="{{ route('admin.fee-payments.update', $item->id) }}">
            @csrf @method('PUT')
            ...
            <div class="modern-form-section">
                <div class="modern-form-section-header">
                    ...
                    <div>
                        <h3 class="modern-form-section-title">Payment Details</h3>
                        <p class="modern-form-section-desc">Update the payment and student information</p>
                    </div>
                </div>
                <div class="modern-form-section-body">
                    <div class="modern-form-grid">
                        <div class="modern-form-group">
                            <label class="modern-form-label" for="fee_id">
                                Fee <span class="modern-required">*</span>
                            </label>
                            <div class="modern-input-wrapper">
                                <i class="fas fa-file-invoice-dollar modern-input-icon"></i>
                                <select name="fee_id" id="fee_id" class="modern-input modern-select {{ $errors->has('fee_id') ? 'is-invalid' : '' }}" required>
                                    <option value="">-- Select Fee --</option>
                                    @foreach(\App\Models\Fee::with('classroom')->orderBy('created_at', 'desc')->get() as $fee)
                                        <option value="{{ $fee->id }}" {{ old('fee_id', $item->fee_id) == $fee->id ? 'selected' : '' }}>
                                            {{ ucfirst($fee->fee_type) }} - {{ $fee->classroom->name ?? 'N/A' }} ({{ number_format($fee->amount, 2) }})
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="modern-form-group">
                            ...
                            <div class="modern-input-wrapper">
                                <i class="fas fa-user-graduate modern-input-icon"></i>
                        ...
                        <div class="modern-form-group">
                            <label class="modern-form-label" for="amount_paid">
                                Amount Paid <span class="modern-required">*</span>
                            </label>
                            <div class="modern-input-wrapper">
                                ...
                                <input type="number"
                                    ...
                                    step="0.01"
                                    min="0"
                                    class="modern-input {{ $errors->has('amount_paid') ? 'is-invalid' : '' }}"
                                    value="{{ old('amount_paid', $item->amount_paid) }}"
                        ...
                        <div class="modern-form-group">
                            <label class="modern-form-label" for="payment_method">
                                Payment Method <span class="modern-required">*</span>
                            </label>
                            <div class="modern-input-wrapper">
                                <i class="fas fa-credit-card modern-input-icon"></i>
                                <select name="payment_method" id="payment_method" class="modern-input modern-select {{ $errors->has('payment_method') ? 'is-invalid' : '' }}" required>
                                    <option value="">-- Select Method --</option>
                                    <option value="cash" {{ old('payment_method', $item->payment_method) == 'cash' ? 'selected' : '' }}>Cash</option>
                                    <option value="bank" {{ old('payment_method', $item->payment_method) == 'bank' ? 'selected' : '' }}>Bank Transfer</option>
                                    <option value="mobile" {{ old('payment_method', $item->payment_method) == 'mobile' ? 'selected' : '' }}>Mobile Payment</option>
                                    <option value="cheque" {{ old('payment_method', $item->payment_method) == 'cheque' ? 'selected' : '' }}>Cheque</option>
                                    <option value="online" {{ old('payment_method', $item->payment_method) == 'online' ? 'selected' : '' }}>Online Payment</option>
                                </select>
                            </div>
                        ...
                        <div class="modern-form-group">
                            ...
                            <div class="modern-input-wrapper">
                                ...
                                <select name="status" id="status" class="modern-input modern-select {{ $errors->has('status') ? 'is-invalid' : '' }}" required>
                                    ...
                                    <option value="partial" {{ old('status', $item->status) == 'partial' ? 'selected' : '' }}>Partial</option>
            ...
            <div class="modern-form-section">
                <div class="modern-form-section-header">
                    ...
                    <div>
                        <h3 class="modern-form-section-title">Transaction Information</h3>
                        <p class="modern-form-section-desc">Receipt and transaction reference details</p>
                    </div>
                </div>
                <div class="modern-form-section-body">
                    <div class="modern-form-grid">
                        <div class="modern-form-group">
                            <label class="modern-form-label" for="receipt_number">
                                Receipt Number <small>(optional)</small>
                            </label>
                            <div class="modern-input-wrapper">
                                <i class="fas fa-barcode modern-input-icon"></i>
                                <input type="text"
                                    name="receipt_number"
                                    id="receipt_number"
                                    class="modern-input {{ $errors->has('receipt_number') ? 'is-invalid' : '' }}"
                                    value="{{ old('receipt_number', $item->receipt_number) }}"
                        ...
                        <div class="modern-form-group">
                            <label class="modern-form-label" for="transaction_id">
                                Transaction ID <small>(optional)</small>
                            </label>
                            <div class="modern-input-wrapper">
                                <i class="fas fa-fingerprint modern-input-icon"></i>
                                <input type="text"
                                    name="transaction_id"
                                    id="transaction_id"
                                    class="modern-input {{ $errors->has('transaction_id') ? 'is-invalid' : '' }}"
                                    value="{{ old('transaction_id', $item->transaction_id) }}"
            ...
            <div class="modern-form-actions">
                <a href="{{ route('admin.fee-payments.index') }}" class="btn-modern btn-modern-ghost">
                    Cancel
                </a>
