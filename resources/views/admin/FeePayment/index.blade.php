    ...
    <div class="modern-stats-row">
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <span class="modern-stat-value">{{ $totalPayments ?? $data->total() }}</span>
                <span class="modern-stat-label">Total Payments</span>
            </div>
        </div>
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <span class="modern-stat-value">{{ number_format($totalCollected ?? 0, 2) }}</span>
    ...
    <div class="modern-card">
        <div class="modern-card-header">
            <div class="modern-card-header-left">
                <h2 class="modern-card-title">All Payments</h2>
                <span class="modern-badge modern-badge-light">{{ $data->total() }} records</span>
            </div>
            <div class="modern-card-header-right">
                <div class="modern-search-box">
                    <i class="fas fa-search"></i>
                    <input type="text" id="paymentSearch" placeholder="Search payments..." onkeyup="filterTable()">
                </div>
            </div>
        </div>
        <div class="modern-card-body">
                ...
                <table class="modern-table" id="paymentTable">
                    <thead>
                        <tr>
                            ...
                            <th>Student</th>
                            <th>Fee Type</th>
                            <th class="th-center">Amount Paid</th>
                            <th class="th-center">Method</th>
                    ...
                    <tbody>
                        ...
                        <tr class="modern-table-row">
                            ...
                            <td>
                                <div class="modern-cell-title">{{ $item->receipt_number ?? '-' }}</div>
                                @if($item->transaction_id)
                                    <div class="modern-cell-sub">TXN: {{ $item->transaction_id }}</div>
                                @endif
                            </td>
                            <td>
                            ...
                            <td class="td-center">
                                <span class="modern-cell-amount">{{ number_format($item->amount_paid, 2) }}</span>
                            </td>
                            <td class="td-center">
                                @switch($item->payment_method)
                                    @case('cash')
                                        ...
                                        @break
                                    @case('bank')
                                        <span class="modern-badge modern-badge-blue">Bank</span>
                                        @break
                                    @case('mobile')
                                        <span class="modern-badge modern-badge-purple">Mobile</span>
                                        @break
                                    @case('cheque')
                                        <span class="modern-badge modern-badge-gold">Cheque</span>
                                        @break
                                    @case('online')
                                        <span class="modern-badge modern-badge-info">Online</span>
                                        @break
                                    @default
                            ...
                            <td class="td-center">
                                <div class="modern-cell-text">{{ $item->payment_date ? $item->payment_date->format('M d, Y') : '-' }}</div>
                            </td>
                            <td class="td-center">
                                @switch($item->status)
                                    @case('paid')
                                        ...
                                        @break
                                    @case('partial')
                                        <span class="modern-badge modern-badge-gold">Partial</span>
                                        @break
                                    @case('pending')
                            ...
                            <td class="td-actions">
                                <div class="modern-action-group">
                                    <a href="{{ route('admin.fee-payments.edit', $item->id) }}" class="modern-btn-icon modern-btn-edit" title="Edit">
                                        <i class="fas fa-pen"></i>
                                    </a>
                                    <form method="POST" action="{{ route('admin.fee-payments.destroy', $item->id) }}" style="display:inline" onsubmit="return confirm('Are you sure you want to delete this payment?')">
                                        @csrf @method('DELETE')
            ...
            <div class="modern-empty-state">
                ...
                <p>Get started by recording your first fee payment.</p>
                <a href="{{ route('admin.fee-payments.create') }}" class="btn-modern btn-modern-primary">
...
.modern-badge-info { background: #ecfeff; color: #0891b2; }
