        ...
        <div class="modern-page-header-left">
            <nav class="modern-breadcrumb">
                <ol>
                    ...
                    <li><a href="{{ route('admin.classrooms.index') }}">Academics</a></li>
                    <li class="active">Details</li>
                </ol>
            </nav>
        </div>
        <div class="modern-page-header-right">
            <a href="{{ route('admin.classrooms.index') }}" class="btn-modern btn-modern-outline"><i class="fas fa-arrow-left"></i><span>Back to List</span></a>
            <a href="{{ route('admin.classrooms.edit', $item->id) }}" class="btn-modern btn-modern-primary"><i class="fas fa-pen"></i><span>Edit</span></a>
        </div>
    </div>
    ...
    <div class="modern-detail-grid">
        <div class="modern-card modern-detail-main">
            <div class="modern-detail-hero">
                <div class="modern-detail-hero-icon"><i class="fas fa-chalkboard"></i></div>
                <div class="modern-detail-hero-info">
                    ...
                    <div class="modern-detail-hero-badges">
                        ...
                        @else
                            <span class="modern-badge modern-badge-light"><i class="fas fa-layer-group"></i> No Sections</span>
                        @endif
                    </div>
                </div>
            </div>
            <div class="modern-detail-body">
                ...
                <div class="modern-detail-row">
                    <div class="modern-detail-label"><i class="fas fa-chalkboard"></i> Class Name</div>
                    <div class="modern-detail-value">{{ $item->name ?? '-' }}</div>
                </div>
                @if($item->numeric_name)
                <div class="modern-detail-row">
                    <div class="modern-detail-label"><i class="fas fa-sort-numeric-up"></i> Numeric Name</div>
                    <div class="modern-detail-value">{{ $item->numeric_name }}</div>
                </div>
                @endif
                <div class="modern-detail-row">
                    <div class="modern-detail-label"><i class="fas fa-calendar-alt"></i> Academic Year</div>
                ...
                @if($item->branch)
                <div class="modern-detail-row">
                    <div class="modern-detail-label"><i class="fas fa-building"></i> Branch</div>
                    <div class="modern-detail-value">{{ $item->branch->name }}</div>
                </div>
                @endif
                <div class="modern-detail-row">
                    <div class="modern-detail-label"><i class="fas fa-layer-group"></i> Sections</div>
                    <div class="modern-detail-value">
                        @if($item->sections && $item->sections->count() > 0)
                            <div class="modern-section-list">
                                @foreach($item->sections as $section)
                                    <span class="modern-badge modern-badge-info">
                                        {{ $section->name }}
                                        @if($section->max_students)
                                            <span style="opacity:0.7">({{ $section->max_students }})</span>
                                        @endif
                                    </span>
                                @endforeach
                            </div>
                        @else
                            <span class="modern-muted">No sections assigned</span>
                        @endif
                    </div>
                </div>
            </div>
        ...
        <div class="modern-detail-sidebar">
            <div class="modern-card">
                ...
                <div class="modern-quick-actions">
                    <a href="{{ route('admin.classrooms.edit', $item->id) }}" class="modern-quick-action">
                        <i class="fas fa-pen"></i>
                        <span>Edit Classroom</span>
                    </a>
                    <a href="{{ route('admin.classrooms.index') }}" class="modern-quick-action">
                        <i class="fas fa-list"></i>
                        <span>All Classrooms</span>
                    </a>
                    <form method="POST" action="{{ route('admin.classrooms.destroy', $item->id) }}" onsubmit="return confirm('Are you sure you want to delete this classroom and all its sections?')">
                        @csrf @method('DELETE')
...
.modern-section-list { display: flex; flex-wrap: wrap; gap: 0.35rem; }
