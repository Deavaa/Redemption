...
.rp-input{width:100%;border:1.5px solid #e5e7eb;border-radius:10px;padding:.65rem .9rem;font-size:.9rem;color:#1a1a2e;background:#f9fafb;transition:all .2s}
...
<div class="rp-page">
    <div class="rp-header">
        <div>
            <h1 class="rp-title">Edit Role: {{ $role->display_name }}</h1>
            <p class="rp-subtitle">Modify role details and permissions</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.roles.update', $role) }}">
            ...
            <div class="rp-card-body">
                <div class="rp-form-grid">
                    <div class="rp-form-group">
                        ...
                        <input type="text" class="rp-input" value="{{ $role->name }}" disabled>
                        <small style="color:#9ca3af;font-size:.75rem;margin-top:.25rem">Role name cannot be changed after creation.</small>
                    </div>
                    <div class="rp-form-group">
                        <label class="rp-label">Display Name <span style="color:#ef4444">*</span></label>
                        <input type="text" name="display_name" class="rp-input" value="{{ old('display_name', $role->display_name) }}" required>
                    </div>
                    <div class="rp-form-group" style="grid-column:span 2">
                        <label class="rp-label">Description</label>
                        <textarea name="description" class="rp-textarea" rows="2">{{ old('description', $role->description) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="rp-card">
            <div class="rp-card-head">
                ...
                <h3 class="rp-card-title">Permissions</h3>
            </div>
            <div class="rp-card-body">
                ...
                <div class="rp-perm-module">
                    ...
                    <div class="rp-perm-module-body" id="module-{{ $module }}">
                        ...
                        <div class="rp-perm-item">
                            <label>
                                <input type="checkbox" name="permissions[]" value="{{ $perm->id }}" class="module-{{ $module }}" {{ in_array($perm->id, $rolePermissionIds) ? 'checked' : '' }}>
                                {{ $perm->display_name }}
        ...
        <div class="rp-form-actions">
            ...
            <button type="submit" class="rp-btn"><i class="fas fa-save"></i> Update Role</button>
        </div>
    </form>
</div>
@endsection
