    ...
    <div class="modern-stats-row">
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <span class="modern-stat-value">{{ number_format($data->where('type', 'income')->sum('amount'), 2) }}</span>
        ...
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <span class="modern-stat-value">{{ number_format($data->where('type', 'expense')->sum('amount'), 2) }}</span>
                <span class="modern-stat-label">Total Expenses</span>
            </div>
        </div>
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                @php $net = $data->where('type', 'income')->sum('amount') - $data->where('type', 'expense')->sum('amount'); @endphp
                <span class="modern-stat-value">{{ number_format($net, 2) }}</span>
    ...
    <div class="modern-card">
        <div class="modern-card-header">
            <div class="modern-card-header-left">
                <h2 class="modern-card-title">All Records</h2>
                <span class="modern-badge modern-badge-light">{{ $data->total() }} records</span>
            </div>
            <div class="modern-card-header-right">
                <div class="modern-search-box">
                    <i class="fas fa-search"></i>
                    <input type="text" id="ieSearch" placeholder="Search records..." onkeyup="filterTable()">
                </div>
            </div>
        </div>
        <div class="modern-card-body">
            ...
            <div class="modern-table-wrapper">
                <table class="modern-table" id="ieTable">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        ...
                        <tr class="modern-table-row">
                            ...
                            <td class="td-center">
                                @if($item->type === 'income')
                                    <span class="modern-badge modern-badge-success">Income</span>
                                @else
                                    <span class="modern-badge modern-badge-danger">Expense</span>
                                @endif
                            </td>
                            <td>
                                <div class="modern-cell-title">{{ ucfirst($item->category ?? '-') }}</div>
                                @if($item->description)
                                    <div class="modern-cell-sub">{{ \Illuminate\Support\Str::limit($item->description, 35) }}</div>
                                @endif
                            </td>
                            <td class="td-center">
                                @if($item->type === 'income')
                                    <span class="modern-cell-text-green">+ {{ number_format($item->amount, 2) }}</span>
                                @else
                                    <span class="modern-cell-text-red">- {{ number_format($item->amount, 2) }}</span>
                                @endif
                            </td>
                            <td>
                                <div class="modern-cell-text">{{ $item->date ? $item->date->format('M d, Y') : '-' }}</div>
                            </td>
                            <td>
                                <div class="modern-cell-text">{{ $item->reference ?? '-' }}</div>
                            </td>
                            <td>
                                <div class="modern-cell-text">{{ $item->branch->name ?? '-' }}</div>
                            </td>
                            <td class="td-actions">
                                <div class="modern-action-group">
                                    <a href="{{ route('admin.income-expenses.edit', $item->id) }}" class="modern-btn-icon modern-btn-edit" title="Edit">
                                        <i class="fas fa-pen"></i>
                                    </a>
                                    <form method="POST" action="{{ route('admin.income-expenses.destroy', $item->id) }}" style="display:inline" onsubmit="return confirm('Are you sure you want to delete this record?')">
                                        @csrf @method('DELETE')
            ...
            <div class="modern-empty-state">
                ...
                <p>Get started by adding your first income or expense record.</p>
                <a href="{{ route('admin.income-expenses.create') }}" class="btn-modern btn-modern-primary">
...
function filterTable() {
    const input = document.getElementById('ieSearch');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('ieTable');
