...
@section('title', 'Slider Alert Messages')
        ...
        <div class="modern-page-header-left">
            <nav aria-label="breadcrumb" class="modern-breadcrumb">
                <ol>
                    ...
                    <li class="active">Slider Alerts</li>
                </ol>
            </nav>
        </div>
        <div class="modern-page-header-right">
            <a href="{{ route('admin.slider-alerts.create') }}" class="btn-modern btn-modern-primary">
    ...
    <div class="modern-stats-row">
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <span class="modern-stat-value">{{ $totalAlerts }}</span>
                <span class="modern-stat-label">Total Alerts</span>
            </div>
        </div>
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <span class="modern-stat-value">{{ $activeAlerts }}</span>
        ...
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <span class="modern-stat-value">{{ $inactiveAlerts }}</span>
                <span class="modern-stat-label">Inactive</span>
            </div>
        </div>
    </div>

    {{-- Alert Messages Table Card --}}
    <div class="modern-card">
        <div class="modern-card-header">
            <div class="modern-card-header-left">
                <h2 class="modern-card-title">Slider Alert Messages</h2>
                <span class="modern-badge modern-badge-light">{{ $data->total() }} records</span>
            </div>
            <div class="modern-card-header-right">
                <div class="modern-search-box">
                    <i class="fas fa-search"></i>
                    <input type="text" id="alertSearch" placeholder="Search alerts..." onkeyup="filterTable()">
                </div>
            </div>
        </div>
        <div class="modern-card-body">
            ...
            <div class="modern-table-wrapper">
                <table class="modern-table" id="alertTable">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        ...
                        <tr class="modern-table-row">
                            ...
                            <td>
                                <span style="display:inline-flex;align-items:center;gap:6px;padding:4px 12px;border-radius:50px;font-size:0.75rem;font-weight:600;background:{{ $item->bg_color }};color:{{ $item->text_color }};white-space:nowrap;">
                                    <i class="fas {{ $item->icon }}" style="font-size:0.7rem;"></i>
                                    {{ \Illuminate\Support\Str::limit($item->message, 30) }}
                                </span>
                            </td>
                            <td>
                                <div class="modern-cell-title">{{ $item->message }}</div>
                            </td>
                            <td>
                            ...
                            <td class="td-center">
                                <button type="button" class="modern-toggle-btn {{ $item->is_active ? 'is-active' : '' }}"
                                    onclick="toggleAlert({{ $item->id }})" title="{{ $item->is_active ? 'Click to deactivate' : 'Click to activate' }}">
                                    <span class="modern-toggle-btn-dot"></span>
                                </button>
                            </td>
                            <td class="td-actions">
                                <div class="modern-action-group">
                                    <a href="{{ route('admin.slider-alerts.edit', $item->id) }}" class="modern-btn-icon modern-btn-edit" title="Edit">
                                        <i class="fas fa-pen"></i>
                                    </a>
                                    <form method="POST" action="{{ route('admin.slider-alerts.destroy', $item->id) }}" style="display:inline" onsubmit="return confirm('Are you sure you want to delete this alert?')">
                                        @csrf @method('DELETE')
            ...
            <div class="modern-empty-state">
                ...
                <h3>No Alert Messages Yet</h3>
                <p>Add alert messages to display on the homepage slider (e.g. "Registration is open for 2026/2027").</p>
                <a href="{{ route('admin.slider-alerts.create') }}" class="btn-modern btn-modern-primary">
...
.modern-stat-icon-gold { background: #fff7ed; color: #f97316; }
...
.modern-toggle-btn.is-active {
    background: #059669;
}

.modern-toggle-btn-dot {
    position: absolute;
    top: 3px;
    left: 3px;
    width: 18px;
...
.modern-toggle-btn.is-active .modern-toggle-btn-dot {
...
function toggleAlert(id) {
    ...
    .then(data => {
        if (data.success) {
            ...
            const row = document.querySelector(`form[action*="${id}"]`)?.closest('tr');
            if (row) {
                const btn = row.querySelector('.modern-toggle-btn');
                if (btn) {
                    btn.classList.toggle('is-active', data.is_active);
                    btn.title = data.is_active ? 'Click to deactivate' : 'Click to activate';
                }
            }
        }
    })
