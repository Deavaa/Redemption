...
@section('title', 'Edit Slider Alert')
...
<div class="modern-page">
    ...
    <div class="modern-card">
        <form method="POST" action="{{ route('admin.slider-alerts.update', $item->id) }}" id="alertForm">
            @csrf
            @method('PUT')
                    ...
                    <div class="modern-form-grid">
                        ...
                        <div class="modern-form-group">
                            ...
                            <select name="type" id="type" class="modern-input" style="padding-left:14px;" onchange="applyTypePreset()">
                                <option value="info" {{ old('type', $item->type) === 'info' ? 'selected' : '' }}>Info (Green)</option>
                                <option value="success" {{ old('type', $item->type) === 'success' ? 'selected' : '' }}>Success</option>
                                <option value="warning" {{ old('type', $item->type) === 'warning' ? 'selected' : '' }}>Warning (Amber)</option>
                                <option value="danger" {{ old('type', $item->type) === 'danger' ? 'selected' : '' }}>Danger (Red)</option>
                                <option value="primary" {{ old('type', $item->type) === 'primary' ? 'selected' : '' }}>Primary (Blue)</option>
                            </select>
                            @error('type')
                                <span class="modern-form-error">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="modern-form-group">
                            ...
                            <div style="display:flex;align-items:center;gap:10px;">
                                <input type="color" name="text_color" id="text_color"
                                    value="{{ old('text_color', $item->text_color) }}"
                                    style="width:44px;height:38px;border:1.5px solid #e5e7eb;border-radius:8px;cursor:pointer;padding:2px;"
                                    oninput="updatePreview()">
                                <div class="modern-input-wrapper" style="flex:1;">
                        ...
                        <div class="modern-form-group">
                            <div class="modern-toggle-wrapper">
                                <label class="modern-toggle">
                                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $item->is_active ? '1' : '0') == '1' ? 'checked' : '' }}>
                ...
                <div class="modern-form-section-body">
                    <div style="background:linear-gradient(135deg,#1a1a2e,#0f0f23);border-radius:12px;padding:20px;display:flex;align-items:center;gap:16px;">
                        <div style="flex:1;overflow:hidden;">
                            <span id="previewItem" style="display:inline-flex;align-items:center;gap:8px;padding:6px 18px;border-radius:50px;font-size:0.82rem;font-weight:600;background:{{ $item->bg_color }};color:{{ $item->text_color }};white-space:nowrap;">
                                <i id="previewIcon" class="fas {{ $item->icon }}" style="font-size:0.78rem;"></i>
                                <span id="previewText">{{ $item->message }}</span>
                            </span>
                        </div>
                        <div style="background:rgba(0,0,0,0.25);padding:8px 14px;border-radius:50px;">
                            <span id="previewBadge" style="display:inline-flex;align-items:center;gap:8px;padding:6px 16px;border-radius:50px;font-size:0.82rem;font-weight:700;background:{{ $item->bg_color }};color:{{ $item->text_color }};white-space:nowrap;">
                                <i id="previewBadgeIcon" class="fas {{ $item->icon }}" style="font-size:0.78rem;"></i>
                                <span id="previewBadgeText">{{ $item->message }}</span>
                            </span>
                        </div>
                    </div>
                </div>
