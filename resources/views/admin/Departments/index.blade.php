...
<div class="modern-page">
    <div class="modern-page-header">
        ...
        <div class="modern-page-header-right">
            ...
            <a href="{{ route('admin.departments.create') }}" class="btn-modern btn-modern-primary">
    ...
    <div class="modern-card">
        <div class="modern-card-header">
            <div class="modern-card-header-left">
                <h2 class="modern-card-title">All Departments</h2>
                <span class="modern-badge modern-badge-light">{{ $departments->total() }} records</span>
            </div>
        </div>
        <div class="modern-card-body">
                ...
                <table class="modern-table">
                    <thead>
                        <tr>
                            ...
                            <th class="th-center">Teachers</th>
                    ...
                    <tbody>
                        ...
                        <tr class="modern-table-row">
                            <td class="td-narrow">
                                <span class="modern-row-number">{{ $loop->iteration + ($departments->currentPage() - 1) * $departments->perPage() }}</span>
                            </td>
                            <td>
                                <div class="modern-cell-title">{{ $dept->name }}</div>
                                @if($dept->description)
                                <div class="modern-cell-sub">{{ Str::limit($dept->description, 60) }}</div>
                                @endif
                            </td>
                            <td>
                                @if($dept->headUser)
                                <div class="modern-cell-text">{{ $dept->headUser->name }}</div>
                                @else
                                <span class="modern-cell-muted">Not assigned</span>
                                @endif
                            </td>
                            <td class="td-center">
                                <span class="modern-badge modern-badge-info">{{ $dept->teachers->count() }}</span>
                            </td>
                            <td class="td-center">
                                @if($dept->is_active)
                            ...
                            <td class="td-actions">
                                <div class="modern-action-group">
                                    <a href="{{ route('admin.departments.show', $dept->id) }}" class="modern-btn-icon modern-btn-view" title="View">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="{{ route('admin.departments.edit', $dept->id) }}" class="modern-btn-icon modern-btn-edit" title="Edit">
                                        <i class="fas fa-pen"></i>
                                    </a>
                                    <form method="POST" action="{{ route('admin.departments.destroy', $dept->id) }}" style="display:inline" onsubmit="return confirm('Delete this department?')">
                                        @csrf @method('DELETE')
            ...
            <div class="modern-empty-state">
                <div class="modern-empty-icon"><i class="fas fa-building"></i></div>
                <h3>No Departments</h3>
                <p>Create departments to organize teachers and manage exam question reviews.</p>
                @if(in_array(Auth::user()->role, ['admin', 'super_admin']))
                <a href="{{ route('admin.departments.create') }}" class="btn-modern btn-modern-primary"><i class="fas fa-plus"></i> Add Department</a>
                @endif
            </div>
            @endif
        </div>
...
.modern-card { background:#fff; border-radius:14px; box-shadow:0 1px 3px rgba(0,0,0,.06); border:1px solid #f0f0f0; overflow:hidden; margin-bottom:1.5rem; }
...
.modern-cell-sub { font-size:.8rem; color:#9ca3af; margin-top:2px; }
.modern-cell-text { color:#4b5563; }
.modern-cell-muted { color:#d1d5db; font-size:.85rem; }
...
.modern-alert { display:flex; align-items:center; gap:.65rem; padding:.85rem 1.25rem; margin:1rem 1.5rem; border-radius:10px; font-size:.88rem; font-weight:500; }
.modern-alert-success { background:#ecfdf5; color:#059669; border:1px solid #a7f3d0; }
.modern-alert-close { margin-left:auto; background:none; border:none; cursor:pointer; color:inherit; opacity:.6; }
