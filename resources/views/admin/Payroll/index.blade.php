...
<div class="modern-page">
    ...
    <div class="modern-page-header">
        <div class="modern-page-header-left">
            ...
            <h1 class="modern-page-title">Payroll</h1>
            <p class="modern-page-subtitle">Manage employee salaries and payroll processing</p>
        </div>
        <div class="modern-page-header-right">
            <a href="{{ route('admin.payrolls.create') }}" class="btn-modern btn-modern-primary">
    ...
    <div class="modern-stats-row">
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                ...
                <span class="modern-stat-label">Total Records</span>
            </div>
        </div>
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <span class="modern-stat-value">{{ number_format($data->sum('net_salary'), 2) }}</span>
                <span class="modern-stat-label">Total Net Salary</span>
            </div>
        </div>
        <div class="modern-stat-card">
            ...
            <div class="modern-stat-info">
                <span class="modern-stat-value">{{ $data->where('status', 'paid')->count() }}</span>
                <span class="modern-stat-label">Paid</span>
            </div>
        </div>
    </div>
    ...
    <div class="modern-card">
        <div class="modern-card-header">
            <div class="modern-card-header-left">
                <h2 class="modern-card-title">All Payroll Records</h2>
                <span class="modern-badge modern-badge-light">{{ $data->total() }} records</span>
            </div>
            <div class="modern-card-header-right">
                <div class="modern-search-box">
                    <i class="fas fa-search"></i>
                    <input type="text" id="payrollSearch" placeholder="Search payroll..." onkeyup="filterTable()">
                </div>
            </div>
        </div>
        <div class="modern-card-body">
                ...
                <table class="modern-table" id="payrollTable">
                    <thead>
                        <tr>
                            ...
                            <th>Employee</th>
                            <th class="th-center">Basic Salary</th>
                            <th class="th-center">Allowances</th>
                            <th class="th-center">Deductions</th>
                            <th class="th-center">Net Salary</th>
                            <th class="th-center">Period</th>
                    ...
                    <tbody>
                        ...
                        <tr class="modern-table-row">
                            ...
                            <td>
                                <div class="modern-cell-title">{{ $item->employee->name ?? '-' }}</div>
                                <div class="modern-cell-sub">{{ $item->employee->email ?? '' }}</div>
                            </td>
                            <td class="td-center">
                                <span class="modern-cell-amount">{{ number_format($item->basic_salary, 2) }}</span>
                            </td>
                            <td class="td-center">
                                <span class="modern-cell-text-green">{{ number_format($item->allowances, 2) }}</span>
                            </td>
                            <td class="td-center">
                                <span class="modern-cell-text-red">{{ number_format($item->deductions + $item->tax, 2) }}</span>
                            </td>
                            <td class="td-center">
                                <span class="modern-cell-amount">{{ number_format($item->net_salary, 2) }}</span>
                            </td>
                            <td class="td-center">
                                <div class="modern-cell-text">{{ $item->pay_period ?? '-' }}</div>
                                <div class="modern-cell-sub">{{ $item->payment_date ? $item->payment_date->format('M d, Y') : '' }}</div>
                            </td>
                            <td class="td-center">
                                @switch($item->status)
                                    @case('paid')
                                        <span class="modern-badge modern-badge-success">Paid</span>
                                        @break
                                    @case('processed')
                                        <span class="modern-badge modern-badge-blue">Processed</span>
                                        @break
                                    @case('pending')
                                        <span class="modern-badge modern-badge-light">Pending</span>
                                        @break
                                    @default
                            ...
                            <td class="td-actions">
                                <div class="modern-action-group">
                                    <a href="{{ route('admin.payrolls.edit', $item->id) }}" class="modern-btn-icon modern-btn-edit" title="Edit">
                                        <i class="fas fa-pen"></i>
                                    </a>
                                    <form method="POST" action="{{ route('admin.payrolls.destroy', $item->id) }}" style="display:inline" onsubmit="return confirm('Are you sure you want to delete this payroll record?')">
                                        @csrf @method('DELETE')
            ...
            <div class="modern-empty-state">
                ...
                <p>Get started by adding your first payroll record.</p>
                <a href="{{ route('admin.payrolls.create') }}" class="btn-modern btn-modern-primary">
