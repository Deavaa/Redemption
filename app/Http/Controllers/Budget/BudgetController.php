...
class BudgetController extends Controller
{
    ...
    {
        $q = Budget::with(['academicYear', 'branch']);
        if ($r->filled('search')) {
            $s = $r->search;
        ...
        if ($r->filled('academic_year_id')) {
            $q->where('academic_year_id', $r->academic_year_id);
        }
        ...
        return view('admin.Budget.index', compact('data', 'totalBudgets', 'totalAllocated', 'totalSpent', 'academicYears', 'branches'));
    }
    ...
    public function create()
    {
        $academicYears = AcademicYear::orderBy('name')->get();
        $branches = Branch::orderBy('name')->get();
        return view('admin.Budget.create', compact('academicYears', 'branches'));
    }
    ...
    public function store(Request $r)
    {
        $r->validate([
            ...
            'budget_code' => 'nullable|string|max:255|unique:budgets,budget_code',
    ...
    {
        $budget->load(['academicYear', 'branch']);
    ...
    public function edit(Budget $budget)
    {
        $academicYears = AcademicYear::orderBy('name')->get();
        $branches = Branch::orderBy('name')->get();
        return view('admin.Budget.edit', ['item' => $budget, 'academicYears' => $academicYears, 'branches' => $branches]);
    }
    ...
    public function update(Request $r, Budget $budget)
    {
        $r->validate([
            'branch_id' => 'nullable|exists:branches,id',
            'budget_code' => 'nullable|string|max:255|unique:budgets,budget_code,' . $budget->id,
            'academic_year_id' => 'required|exists:academic_years,id',
            'category' => 'required|string|max:255',
            'allocated_amount' => 'required|numeric|min:0',
            'spent_amount' => 'nullable|numeric|min:0',
